...
const trainerTemplate = document.getElementById("trainer-card");
const modalTemplate = document.getElementById("modal-template");
const sortAYA = document.querySelectorAll(".sorting__btn")[0];
const sortLastName = document.querySelectorAll(".sorting__btn")[1];
const sortExperience = document.querySelectorAll(".sorting__btn")[2];

function renderTrainers(trainerList) {
	container.innerHTML = ""; 
	trainerList.forEach(displayTrainer);
}

function displayTrainer(trainerInfo) {
	const trainer = trainerTemplate.content.cloneNode(true); 

	trainer.querySelector(".trainer__img").src = trainerInfo.photo;
	trainer.querySelector(".trainer__img").alt = `${trainerInfo["first name"]} ${trainerInfo["last name"]}`;
	trainer.querySelector(".trainer__name").textContent = `${trainerInfo["first name"]} ${trainerInfo["last name"]}`;
	
	trainer.querySelector(".trainer__show-more").addEventListener("click", () => {
        showModal(trainerInfo);
    });
	container.appendChild(trainer); 
	
}

function showModal(trainerInfo){
	...
	modal.querySelector(".modal__img").src = trainerInfo.photo;
	modal.querySelector(".modal__img").alt = `${trainerInfo["first name"]} ${trainerInfo["last name"]}`;
	modal.querySelector(".modal__name").textContent = `${trainerInfo["first name"]} ${trainerInfo["last name"]}`;
	modal.querySelector(".modal__point--category").textContent = `Категорія: ${trainerInfo.category}`;
	modal.querySelector(".modal__point--experience").textContent = `Досвід: ${trainerInfo.experience}`;
	modal.querySelector(".modal__point--specialization").textContent = trainerInfo.specialization;
	modal.querySelector(".modal__text").textContent = trainerInfo.description;
...
sortExperience.addEventListener("click", function(){
